...
resource "null_resource" "annotate-ebs-csi-controller" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      # ---------------------------------------
      kubectl annotate serviceaccount ebs-csi-controller-sa -n kube-system eks.amazonaws.com/role-arn=arn:aws:iam::${var.aws_account_id}:role/${aws_iam_role.AmazonEKS_EBS_CSI_DriverRole.name}
